...
docker exec -u hadoop -d nodemaster mr-jobhistory-daemon.sh start historyserver
  sleep 5
  echo ">> Starting Spark ..."
  docker exec -u hadoop -d node2 start-master.sh
  ...
  docker exec -u hadoop node2 start-history-server.sh
  sleep 5
...
if [[ $1 = "install" ]]; then
  ...
  docker run -d --net hadoopnet --ip 172.20.1.4 --hostname psqlhms --name psqlhms -e POSTGRES_PASSWORD=hive -it madhudharmapuri/hadoop-3node-spark-hive-cluster:postgresql
  sleep 5
...
if [[ $1 = "pull_images" ]]; then  
  docker pull -a madhudharmapuri/hadoop-3node-spark-hive-cluster
  exit
fi
